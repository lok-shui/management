...
export default {
  ...
  mounted() {
    ...
    // 新方法
    this.getUserFeedbackList(this.queryParam);
  },
  methods: {
    userFeedbackList() {
      this.loading = true;
    ...
    passApplication(record) {
      ...
      this.$confirm({
        ...
        onOk: () => {
          ...
          this.getUserFeedbackId(record.id, parameter);
        },
        onCancel() {}
      });
    },
    ...
    handleTableChange(pagination) {
      ...
      this.queryParam.offset = pagination.current;
      this.queryParam.limit = pagination.pageSize;
      // this.userFeedbackList();
      this.getUserFeedbackList(this.queryParam);
    },

    // 接口：列表
    async getUserFeedbackList(parameter) {
      this.loading = true;
      const response = await userService.getUserFeedbackList(parameter);
      this.loading = false;
      // console.log('response',response)
      const pagination = { ...this.pagination };
      pagination.total = response.count;
      const data = response.docs; //这里写动态数组名称
      this.data = data;
      this.pagination = pagination;
    },
    // 接口：搁置/已处理
    async getUserFeedbackId(id, parameter) {
      const response = await userService.getUserFeedbackId(id, parameter);
      console.log("搁置/已处理", response);
    },

    // 接口：查看
    async getUserFeedbackDetail(parameter) {
      const response = await userService.getUserFeedbackDetail(parameter);
      console.log("getUserFeedbackDetail", response);
